...
#Output
kraken_v1_dir=/data/neurogen/Tao/kraken_output
kraken_output_dir=/data/neurogen/Tao/kraken_output_v2
...
if [[ ! -f $output_sample_dir/unmapped.filterLC.filterPhiX.bam ]]; then
	# firstly, filtering reads from PhiX rapidly using SNAP -mrl 20 -F u -d 5(maximum edit distance allowed per read or pair)
	...
	snap-aligner single $phiX_snap_index_path -fastq $output_sample_dir/unmapped.filterLC.fastq -d 5 -mrl 20 -F u \
...
if [[ ! -f $output_sample_dir/unmapped.filterLC.filterPhiX.filterHg38.bam ]]; then
	...
	snap-aligner single $hg38_index_path -d 5 -mrl 20 -F u \
...
if [[ ! -f $output_sample_dir/unmapped.filterLC.filterPhiX.filterHg38.filterLSU.bam ]]; then
	...
	snap-aligner single $LSUrRna_index_path -d 5 -mrl 20 -F u \
...
if [[ ! -f $output_sample_dir/unmapped.filterLC.filterPhiX.filterHg38.filterLSU_SSU.bam ]]; then
	...
	snap-aligner single $SSUrRna_index_path -d 5 -mrl 20 -F u \
...
if [[ ! -f $output_sample_dir/AllCircRnaList.txt ]]; then
	if [[ -f $kraken_v1_dir/$input_sample_name/AllCircRnaList.txt ]]; then
		cp $kraken_v1_dir/$input_sample_name/AllCircRnaList.txt $output_sample_dir
	fi
...
if [[ ! -f $output_sample_dir/AllGeneFusionReads.txt ]]; then
	...
	if [[ -f $kraken_v1_dir/$input_sample_name/AllGeneFusionReads.txt ]]; then
		cp $kraken_v1_dir/$input_sample_name/AllGeneFusionReads.txt $output_sample_dir
	fi
	if [[ $? -ne 0 ]]; then
