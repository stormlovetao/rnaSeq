...
# Sort columns by batch ID # don't see batch related contamination
...
# Sort columns by sample source (where is the sample come from)
table_mat_selected_colnames_splitmat = do.call(rbind, strsplit(colnames(table_mat_selected),'_'))
source_col = table_mat_selected_colnames_splitmat[,2]
match_data = regexpr(pattern = "[A-Z]*", source_col)
source_col = regmatches(source_col, match_data)
source_col[source_col=="B"] = "HBS-B"
source_col[source_col=="H"] = "HBS-H"
source_col[source_col=="M"] = "HBS-M"
table_mat_selected = table_mat_selected[,order(source_col)]
source_col = source_col[order(source_col)]
annotation_col = data.frame(source = source_col)
rownames(annotation_col) = colnames(table_mat_selected)
